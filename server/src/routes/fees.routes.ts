...
// Edit a fee record (amount, due date, status)
router.put('/:id', async (req, res) => {
  const { amount_due, due_date, status } = req.body;
  const { data, error } = await supabase
    .from('fee_records')
    .update({ amount_due, due_date, status })
    .eq('id', req.params.id)
    .select();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data[0]);
...
router.delete('/:id', async (req, res) => {
  const { error } = await supabase.from('fee_records').delete().eq('id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true });
});

// Teacher read-only: fee records for one class
router.get('/by-class/:class', async (req, res) => {
  const { data, error } = await supabase
    .from('fee_records')
    .select('*, students!inner(name, class)')
    .eq('students.class', req.params.class);
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});
